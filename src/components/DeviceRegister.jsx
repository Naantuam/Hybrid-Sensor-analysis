...
export default function DeviceRegister({ toggleSidebar }) {
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-6xl mx-auto p-4">
      ...
      {/* TOP LEVEL WIRELESS ADB CONTROLLER WIZARD */}
      {selectedDevice && (
        selectedDevice.serial.includes(':') ? (
          <div className="bg-[#10111a]/60 border border-emerald-500/25 rounded-2xl p-5 backdrop-blur-md flex flex-col md:flex-row justify-between items-center gap-4 animate-fadeIn">
            <div className="flex items-center gap-3">
              <CheckCircle2 className="w-5 h-5 text-emerald-400" />
              <div>
                <h4 className="text-xs font-bold text-white font-outfit uppercase tracking-wider">Wireless Link Active</h4>
                <p className="text-[10px] text-gray-400 mt-0.5">This handset is successfully connected via wireless ADB ({selectedDevice.serial}). You can toggle the START BRIDGE action below.</p>
              </div>
            </div>
          </div>
        ) : (
          <div className="bg-[#10111a]/60 border border-cyan-500/25 rounded-2xl p-5 backdrop-blur-md flex flex-col md:flex-row justify-between items-center gap-4 animate-fadeIn">
            <div className="flex items-center gap-3">
              <Radio className="w-5 h-5 text-cyan-400 animate-pulse flex-shrink-0" />
              <div>
                <h4 className="text-xs font-bold text-white font-outfit uppercase tracking-wider">Wireless Handoff Control Portal</h4>
                <p className="text-[10px] text-gray-400 mt-0.5">
                  {wirelessHandoffStep === 0 && "Plug in USB and click to enable Wireless TCP on the device."}
                  {wirelessHandoffStep === 1 && "Accessing handset configurations wirelessly..."}
                  {wirelessHandoffStep === 2 && "TCP debugging port unlocked successfully!"}
                  {wirelessHandoffStep === 3 && "Exchanging keys and establishing connection..."}
                  {wirelessHandoffStep === 4 && "Setup complete! Telemetry is active over your hotspot/Wi-Fi."}
                  {wirelessHandoffStep === 5 && "Handset needs authentication. Allow connection in the phone popup."}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-3.5 w-full md:w-auto">
              ...
              {wirelessHandoffStep === 0 && (
                <button
                  ...
                  className="w-full md:w-auto px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold rounded-xl transition-all shadow-md flex items-center justify-center gap-1.5"
                >
              ...
              {wirelessHandoffStep === 1 && (
                <button
                  disabled
                  className="w-full md:w-auto px-4 py-2 bg-cyan-800 text-cyan-400 text-xs font-bold rounded-xl flex items-center justify-center gap-1.5"
                >
              ...
              {/* Step 2: Unplug USB and Connect */}
              {wirelessHandoffStep === 2 && (
                <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                  <span className="text-[11px] font-mono text-cyan-400 bg-cyan-950/40 border border-cyan-800/30 px-3 py-1.5 rounded-lg animate-pulse">
                    ⚠️ Unplug USB, then Click Connect
                  </span>
                  <button
                    onClick={handleConnectWireless}
                    className="w-full md:w-auto px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-emerald-950/20 flex items-center justify-center gap-1.5 animate-bounce"
                  >
                    2. Connect Wi-Fi ({resolvedIp})
                  </button>
                </div>
              )}
              ...
              {wirelessHandoffStep === 3 && (
                <button
                  disabled
                  className="w-full md:w-auto px-4 py-2 bg-emerald-800 text-emerald-400 text-xs font-bold rounded-xl flex items-center justify-center gap-1.5"
                >
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  Pairing wirelessly...
                </button>
              ...
              {wirelessHandoffStep === 4 && (
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-emerald-400 bg-emerald-950/40 border border-emerald-900/30 px-3 py-1.5 rounded-xl flex items-center gap-1">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    Successfully Connected to {resolvedIp}:5555
                  </span>
                  <button
                    onClick={() => setWirelessHandoffStep(0)}
                    className="p-2 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white rounded-lg border border-white/5"
                    title="Reset Wizard"
                  >
              ...
              {/* Step 3: Unauthorized Warning */}
              {wirelessHandoffStep === 5 && (
                <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                  <span className="text-[11px] text-amber-400 bg-amber-950/40 border border-amber-900/30 px-3 py-1.5 rounded-lg font-mono">
                    ⚠️ Check phone & Accept authorization popup!
                  </span>
                  <div className="flex gap-2 w-full md:w-auto">
                    <button
                      onClick={handleConnectWireless}
                      className="w-full md:w-auto px-4 py-2 bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold rounded-xl transition-all shadow-md flex items-center justify-center gap-1.5"
                    >
                      Retry Connection
                    </button>
                    <button
            ...
            <div className="bg-[#10111a]/60 border border-white/5 rounded-2xl p-4 md:p-8 backdrop-blur-md space-y-6">
              ...
              <div className="flex flex-col md:flex-row gap-8 items-center">
                ...
                <div className="flex-1 space-y-4 w-full">
                  ...
                  <div className="text-xs text-gray-300 space-y-2 pt-4 border-t border-white/5 bg-cyan-500/5 p-3 rounded-xl border border-cyan-500/10">
                    <span className="font-bold text-cyan-400 block flex items-center gap-1.5">
                      <Radio className="w-3.5 h-3.5 animate-pulse" />
                      C. Wireless Handoff Mode
                    </span>
                    <p className="text-gray-400 leading-relaxed text-[11px]">
                      Initialize wireless debugging using the control bar at the top of the screen to disconnect the USB cable.
                    </p>
                  </div>

                  <div className="text-xs text-gray-300 space-y-2 pt-4 border-t border-white/5 bg-white/[0.02] p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white block">D. Dynamic Network Failover</span>
                    <p className="text-gray-400 leading-relaxed text-[11px]">
                      Once paired wirelessly, you can toggle **START BRIDGE** to log hardware events. If the phone moves out of Starlink/Wi-Fi range, its local Termux background loop will automatically switch telemetry streams to your remote Railway cloud cluster over cellular data.
                    </p>
                  </div>
                </div>
