...
sections = [
    ("4.1 Introduction", 
     """This chapter presents the operational implementation, empirical testing, and forensic evaluation results of the hybrid mobile sensor monitoring framework. Engineered specifically for modern Android architectures running Android 10 and above (API Level 29+), the platform was deployed and evaluated on a physical smartphone to address the core research problems established in Chapter 1: namely, the Exfiltration Window of static analysis tools, the Runtime Observability Gap in mobile user interfaces, and the Connectivity Dependency of cloud-only security frameworks. Following the Design Science Research Methodology (DSRM) demonstration and evaluation phases, this section details the physical handset testbed configuration, the edge analysis server pipeline, the centralized security dashboard interface, and the empirical testing conducted across real-world application scenarios. Detected security events are quantified, evaluated for false-positive suppression efficiency, and mapped directly to the MITRE ATT&CK Mobile Matrix."""),

    ("4.2 System Setup and Deployment Architecture", 
     """The complete monitoring framework comprises three tightly integrated operational components: a client-side telemetry agent running on the smartphone, an edge analysis server for rule evaluation, and a web-based forensic security dashboard. System specifications mandate native compatibility with Android 10 and above, taking advantage of modern Android 10 AppOps tracking structures and background execution limit enforcement models. Empirical testing was conducted on a physical Infinix X683 smartphone running native Android 10 (API Level 29), connected to the host analysis computer via a high-speed Universal Serial Bus (USB) Android Debug Bridge (ADB) interface."""),

    ("4.2.1 Mobile Telemetry Agent Setup", 
     """The client-side telemetry agent extracts sensor access events directly from Android system logs using low-overhead diagnostic dumpsys commands (dumpsys appops, dumpsys media.camera, and dumpsys location). Operating natively on Android 10 and above, the agent executes continuous polling loops at a calibrated 1.0 Hz sampling frequency while the handset is in active use. It captures sensor access states for hardware camera lenses, acoustic microphone channels, GPS location brokers, and Bluetooth scanning services, recording package names, execution states (foreground or background), and display power states (screen ON or screen OFF). To optimize resource utilization and maintain high system responsiveness, the agent implements adaptive polling logic that throttles background sampling during periods of display dormancy while maintaining lock-free event queues."""),

    ("4.2.2 Edge Server and Real-time Communication", 
     """The edge analysis server was developed using Node.js and Express to deliver high-throughput, low-latency threat evaluation. Addressing the Connectivity Dependency problem highlighted in Chapter 1, the server incorporates a hybrid database architecture. Incoming raw telemetry packets are evaluated against the four-technique threat rules engine and immediately persisted into an embedded SQLite database (sensor_local.db) using write-ahead logging (WAL). When external network connectivity is established, an asynchronous background routine replicates local records to a PostgreSQL database hosted on Neon Cloud. The server integrates a WebSocket broker on TCP port 4444, streaming live telemetry packets and security alerts directly to dashboard clients with sub-second transmission speed."""),

    ("4.2.3 Security Dashboard and Live Monitor", 
     """To resolve the Runtime Observability Gap identified in Chapter 1, the user-facing monitoring interface was constructed using React and Tailwind CSS, creating a centralized security operations center interface. The dashboard is divided into three functional views: a System Overview detailing aggregate threat metrics and severity distributions, a Device Dashboard displaying connection state telemetry and risk score trends, and interactive Forensic Incident Report modal overlays. Figure 4.1 shows the Device Dashboard monitoring the Infinix X683 handset node running Android 10, displaying active USB streaming connection status, threat class distribution bars, and the chronological risk score trend curve."""),
    ...
    ("4.2.4 Real-time Event Console", 
     """To provide immediate visibility into background sensor operations, the dashboard incorporates a real-time event streaming console. Each incoming packet displays an ISO timestamp, handset node identifier, target application package name, accessed sensor, and assigned threat severity level. Figure 4.2 illustrates the live system console output streaming telemetry packets across Critical, Suspicious, and Benign severity levels as they are evaluated by the edge server."""),
    ...
    ("4.3 Empirical Test Scenarios and Results Analysis", 
     """To evaluate how effectively the framework identifies threat patterns and suppresses false security alarms on Android 10 and above, empirical test scenarios were executed on the physical Infinix X683 handset. Testing focused on evaluating four distinct operational behaviours: safe operating system infrastructure exemption, covert background recording during display sleep, system assistant listener interception, and sideloaded package origin risks. The empirical observations from these tests are analyzed below."""),
...
The high proportion of benign classifications (74%) demonstrates effective false-positive suppression. Safe background sensor access by verified system services (Google Play Services and Google UID Shared) was properly identified and reduced to 0 points, preventing alert fatigue during routine handset operation. Figure 4.8 shows the filtered BENIGN log table displaying safe system events recorded at 0 points for forensic audit purposes without raising security alarms."""),
    ...
    ("4.6 Limitations of the Study", 
     """While empirical testing confirms that the framework successfully operates from Android 10 and above (API Level 29+), detects covert sensor activities, and suppresses false security alarms, several study limitations should be acknowledged:
...
3. Focused Test Application Suite: Evaluation focused on open-source background surveillance tools (FadCam) and core operating system infrastructure services (Google Play Services) rather than heavily obfuscated commercial malware binaries.
...
doc.save(docx_path)
print(f"[✓] Saved expanded Chapter 4 into Word Document: {docx_path}")
