...
• Sideloaded APK Origin Risk (ORIGIN_SIDELOADED): +20 points

Total Calculated Risk Score = 30 + 20 + 50 + 15 + 20 = 135 points (CRITICAL).

The 20-point increase from the baseline sub-total of 115 points originates directly from the ORIGIN_SIDELOADED rule, which penalizes applications installed from non-Play Store package installers. Because the final score exceeded the 100-point threshold, the incident was categorized as CRITICAL. Figure 4.5 shows the detailed sub-score breakdown in the Forensic Incident Report, and Figure 4.6 displays the corresponding MITRE ATT&CK Mobile threat mapping for Video Capture (MITRE T1125) and Accessibility Abuse (MITRE T1406)."""),
    ...
    ("4.4.1 Telemetry Processing Volume and Severity Distribution", 
     """During an extended monitoring trial on the Infinix X683 handset (API Level 29+) comprising both attack execution and routine phone usage, the framework processed a total of 317 telemetry packets. Figure 4.7 presents the System Overview Risk Severity Profile recorded by the dashboard:
...
doc.save(docx_path)
print(f"[✓] Saved updated Chapter 4 with all 12 review fixes into Word Document: {docx_path}")
