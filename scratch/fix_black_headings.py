...
# 1. Clean up Chapter 3 battery reference if present around P447
for p in doc.paragraphs:
    if "3. Mobile Battery Overhead: Measured across 1-hour and 6-hour" in p.text:
        p.text = "3. System Resource Overhead: Evaluated qualitatively by verifying low memory utilization and efficient polling loops during active monitoring."
        for run in p.runs:
...
sections = [
    ...
    ("4.3.2 Scenario 2: Covert Background Recording and Screen Sleep (FadCam)", 
     """Scenario 2 evaluated stealth surveillance capabilities using FadCam (com.fadcam), an open-source background camera and audio recording application installed directly via APK. Video and microphone recording were initiated, and the handset's physical power button was pressed to turn the display OFF. The framework detected continuous hardware camera and microphone polling while the handset display was in a dormant state. The threat engine calculated the total risk score by adding individual sub-score indicators:
...
Because the score exceeded the 100-point threshold, the incident was categorized as CRITICAL. Figure 4.5 shows the detailed sub-score breakdown in the Forensic Incident Report, and Figure 4.6 displays the corresponding MITRE ATT&CK Mobile threat mapping for Video Capture (MITRE T1125) and Accessibility Abuse (MITRE T1406)."""),
    ...
    ("4.4 System Performance and Threat Data Quantification", 
     """To evaluate system operational effectiveness, telemetry events recorded during testing were quantified across severity levels and evaluated for real-time delivery speed."""),

    ("4.4.1 Telemetry Processing Volume and Severity Distribution", 
     """During an extended monitoring trial on the Infinix X683 handset comprising both attack execution and routine phone usage, the framework processed a total of 317 telemetry packets. Figure 4.7 presents the System Overview Risk Severity Profile recorded by the dashboard:
...
The high proportion of benign classifications (74%) demonstrates effective false-positive suppression. Safe background sensor access by verified system services (Google Play Services and Google UID Shared) was properly identified and reduced to 0 points, preventing alert fatigue during routine handset operation. Figure 4.8 shows the filtered BENIGN log table displaying safe system events recorded at 0 points for forensic audit purposes without raising security alarms."""),
    ...
    ("4.6 Limitations of the Study", 
     """While empirical testing confirms that the framework successfully detects covert sensor activities and suppresses false security alarms, several limitations should be acknowledged:

1. Single Physical Test Device: Testing was conducted on a single physical smartphone model (Infinix X683 running Android 10). Customized Android ROMs from other manufacturers (such as Samsung One UI or Xiaomi MIUI) may structure dumpsys logging output with slight variations.
2. Android OS Version Scope: Evaluation was performed on Android 10 (API Level 29). Newer Android versions (Android 12+) incorporate visual status bar privacy indicators (green camera/microphone dots) which inform users of active access, though they lack automated risk scoring or persistent forensic log persistence.
...
for item_type, item_content in sections:
    if item_type.startswith("4."):
        ...
        # All headings strictly in BLACK (0,0,0)
        ...
        if os.path.exists(img_path):
            ...
            c_run.font.color.rgb = RGBColor(0, 0, 0) # Black caption
        ...
        c_run.font.name = 'Times New Roman'
        c_run.font.size = Pt(10)
        c_run.font.color.rgb = RGBColor(0, 0, 0) # Black table title
        target_p._element.addprevious(cap_p._element)
...
doc.save(docx_path)
print(f"[✓] Saved black-heading Chapter 4 into Word Document: {docx_path}")
